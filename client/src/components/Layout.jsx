...
export default function Layout() {
  ...
  const getTabItem = (path, icon, label) => {
    const isActive = path === '/' ? location.pathname === '/' : location.pathname.startsWith(path);
    return (
      <Link
        to={path}
        className={`flex flex-col items-center gap-1.5 py-1.5 px-4.5 rounded-2xl transition-all duration-200 select-none ${
          isActive 
            ? 'text-brand-600 dark:text-brand-400 bg-brand-500/10 dark:bg-brand-500/15 scale-105 font-semibold' 
  ...
  const renderBottomNav = () => {
    let thirdTabPath = '/login';
    let thirdTabLabel = 'Login';
    let thirdTabIcon = (
    ...
    if (user) {
      if (user.role === 'admin' || user.role === 'staff') {
        ...
        thirdTabPath = '/bookings';
        thirdTabLabel = 'Bookings';
        thirdTabIcon = (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
    ...
    return (
      <div className="sm:hidden fixed bottom-5 left-1/2 -translate-x-1/2 w-[92%] max-w-sm bg-white/80 dark:bg-slate-900/85 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 py-2 px-4 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_10px_35px_rgba(0,0,0,0.25)] z-50 flex items-center justify-around transition-all duration-300">
        {getTabItem('/', (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
        ...
        {getTabItem('/directory', (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
        ), 'Explore')}

        {getTabItem(thirdTabPath, thirdTabIcon, thirdTabLabel)}
      </div>
    );
  };
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-x-hidden">
        ...
        <div>
          ...
          {user ? (
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={handleThemeToggle}
                className="flex items-center justify-between px-3 py-2 w-full text-left rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-all cursor-pointer"
                type="button"
              >
                <span className="text-slate-750 dark:text-slate-250 text-sm font-semibold flex items-center">
                  {darkMode ? 'Dark Mode' : 'Light Mode'}
                </span>
                <span className="text-slate-500 dark:text-slate-400 p-1.5">
                  {darkMode ? (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" />
                    </svg>
      ...
      {/* Page content */}
      <main key={location.pathname} className="flex-1 relative z-10 animate-fade-in pb-24 sm:pb-0">
        <Outlet />
      </main>
